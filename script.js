$(document).ready(function() {
    $('#welcomeButton').click(function() {
        ...
        $('#start').click(function() {
            $('#plusMinus, #start').hide( "drop", { direction: "down" }, "slow" );
            $('#hit, #stand').show( "drop", { direction: "down" }, "fast" );
            return startGame();
        });
        ...
        $('#doubleDown').click(function() {
            ...
            let cardImage = "<img id=\"back\" alt=\"card image\" src=\"deck/" + cards[firstCard] + ".svg\">";
...
let startGame = function() {
    ...
    let cardImage = "<img id=\"back\" alt=\"card back\" src=\"deck/playing-card-back.jpg\">";
...
let showExitModal = function() {
    ...
    let firstCard = dealer.cards[0];
    $('#dealer').prepend("<img id=\"back\" alt=\"card image\" src=\"deck/" + cards[firstCard] + ".svg\">");
    setTimeout(function() {
        $('#outMessage').show("slow");
...
let humanCard = function() {
    ...
    let newTag = "<img id=\"" +  cards[cardNumber] + "\" alt=\"card image\" src=\"deck/" +  cards[cardNumber] + ".svg\">";
...
let dealerCard = function() {
    let cardNumber = drawCard();
    dealer.cards.push(cardNumber);
    let cardImage = "<img alt=\"card image\" src=\"deck/" + cards[cardNumber] + ".svg\">";
    $('#dealer').append(cardImage);
};
...
let checkHandValue = function(player) {
    ...
    player.cards.forEach(elem => {
        if (elem < 8 && aceIsTen === 0 && points <= 11) {
            points += 11;
            aceIsTen = 1;
        }
        else if (elem < 8) {
